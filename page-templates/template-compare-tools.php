 ...
 *
 * Section order: Platform Opening → Trusted Logos → Compare cards →
 * What Teams Gain (exec-split) → Why Teams Switch → FAQ → Final CTA.
 *
 * @package TestRo
 */
...
/*
 * Hero keeps the start-testing CTA. Closing band is Secondary then Primary
 * (Try for Free stays via allow_on_footer; Get a Demo is the demo CTA).
 */
...
<div class="testro-page-shell testro-page-shell--compare">
				...
				<?php foreach ( $competitors as $competitor ) : ?>
					...
					<li class="testro-compare-tools__card">
						...
						<a class="testro-compare-tools__link" href="<?php echo esc_url( $href ); ?>">
							...
							<span class="testro-compare-tools__cta"><?php esc_html_e( 'View comparison', 'testro' ); ?></span>
						</a>
					</li>
				<?php endforeach; ?>
			</ul>
	...
	<?php
	/* Framer cBYVoFe2I — exec-split numbered steps on white. */
	get_template_part(
	...
	/* Framer OQLF7pwWn — brand Final CTA; Secondary then Primary. */
	get_template_part(
